...
<input type="button" name=Submit52 value="购买"  class='hsgqiehuanshitu'  onClick="javascript:location.href='goumaiadd.jsp?id=${shangpinxinxi.id}';">
<!--jixaaxnnxiu-->
<input type=button name=Submit53 value=评论 class='hsgqiehuanshitu' <%if((String)request.getSession().getAttribute("username")==null){%> onClick="javascript:alert('请您先登陆！');" <%} else {%> onClick="javascript:hsgpinglun('shangpinxinxi','<%=request.getParameter("id")%>')" <%}%> />
<input type=button name=Submit52 value=收藏 class='hsgqiehuanshitu' <%if((String)request.getSession().getAttribute("username")==null){%> onClick="javascript:alert('请您先登陆！');" <%} else {%> onClick="javascript:location.href='jrsc.jsp?id=<%=request.getParameter("id")%>&biao=shangpinxinxi&ziduan=shangpinmingcheng&biaoj=spxx';" <%}%> />
</td></tr>
