...
public class db {
	 ...
	 private String sConnStr = "jdbc:microsoft:sqlserver://localhost:1433;databasename=s2shyyypglxthx1312ANCD"; 
	 
	 //private String dbDriver="com.mysql.jdbc.Driver";
	 //private String sConnStr = "jdbc:mysql://localhost:3306/s2shyyypglxthx1312ANCDmysql?useUnicode=true&amp;amp;amp;amp;amp;characterEncoding=gb2312"; 
	
	 // private String dbDriver="net.sourceforge.jtds.jdbc.Driver";
	// private String sConnStr = "jdbc:jtds:sqlserver://127.0.0.1:1433;databaseName=s2shyyypglxthx1312ANCD"; 
	...
	//查询类
	  ...
	  public List getShouyebht(){
			String sql = "select id,biaoti,shouyetupian from t_xinwentongzhi where shouyetupian<>'' and shouyetupian  like '%.jpg' and leibie='变幻图' order by id desc";
			list = new ArrayList();
			try{
				connect=DriverManager.getConnection(sConnStr,nsa,nmm);
				Statement stmt=connect.createStatement();
		...
		try{
				connect=DriverManager.getConnection(sConnStr,nsa,nmm);
				Statement stmt=connect.createStatement();
				rs=stmt.executeQuery("select * from t_tiezi where bankuai='"+nbk+"' and fid=0");
				while(rs.next()){
					i++;
				}
				return i;
	  ...
	  //读取版块帖子数
		...
		try{
				connect=DriverManager.getConnection(sConnStr,nsa,nmm);
				Statement stmt=connect.createStatement();
				rs=stmt.executeQuery("select * from t_tiezi where fid="+tzid+"");
				while(rs.next()){
					i++;
				}
				return i;
